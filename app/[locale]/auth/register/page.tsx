...
function RegisterForm() {
  ...
  const doGoogleRegister = async () => {
    ...
    if (typeof window !== "undefined" && window.Telegram?.WebApp?.initData) {
      setError(t("errorGoogleNotAvailableInTelegram"))
      return
    }
    const clientId = process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID
    if (!clientId) { setError(t("errorGoogleNotConfigured")); return }
    setLoading(true)
    setError("")
    try {
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      setResendError(e instanceof Error ? e.message : t("errorResendGeneric"))
    } finally {
      setResending(false)
    }
  }
  ...
  const handleCorrectEmail = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setCorrectEmailError(err instanceof Error ? err.message : t("errorCorrectEmailGeneric"))
    } finally {
      ...
      <div className="min-h-screen bg-[#fafafa] flex items-center justify-center px-4 py-12">
        <div className="w-full max-w-md">
          ...
          <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-10 text-center">
            ...
            {emailCorrected && (
              <p className="text-sm text-emerald-600 font-medium mb-2">{t("emailCorrected")}</p>
            )}
            {resent ? (
              <p className="text-sm text-emerald-600 font-medium">{t("resent")}</p>
            ) : (
              <button
            ...
            {!showCorrectEmail ? (
              ...
              <form onSubmit={handleCorrectEmail} className="mt-4 flex flex-col gap-2 text-left">
                <p className="text-xs text-gray-500 font-medium">{t("correctEmailLabel")}</p>
                <input
                  type="email"
                  placeholder={t("correctEmailPlaceholder")}
                  value={newEmail}
                ...
                <div className="flex gap-2">
                  ...
                  >
                    {correctingEmail ? t("saving") : t("save")}
                  </button>
                  <button
                    type="button"
    ...
    <div className="min-h-screen bg-[#fafafa] flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-8">
          ...
          {step === 1 && (
            <div>
              <h1 className="text-2xl font-bold text-gray-900 mb-2">{t("whoAreYou")}</h1>
              <p className="text-gray-500 text-sm mb-6">{t("chooseRoleSubtitle")}</p>
          ...
          {step === 2 && (
            <div>
              ...
              {telegramRedirectUrl && (
                <p className="text-xs text-gray-400 text-center -mt-1.5 mb-3">
                  {t("telegramNotOpened")}{" "}
