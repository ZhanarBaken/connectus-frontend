...
function LoginForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (next) {
        // `next` is a full pathname captured from a real URL (see
        // lib/api.ts's session-expiry redirect) — it already carries
        // whatever locale prefix it had. Pushing it through next-intl's
      ...
      const msg = e instanceof Error ? e.message : t("errorInvalidCredentials")
    ...
    <div className="min-h-screen bg-[#fafafa] flex items-center justify-center px-4">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-gray-900 mt-6 mb-2">{t("welcome")}</h1>
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-8">
          ...
          {/* OAuth buttons — placed on top so the one-tap flow is
              ...
              Gated by !isInTelegram: inside a Mini App, opening the
              bot link from within Telegram is nonsensical, and the
              Mini App auto-login overlay (triggered above) handles
              the flow already. */}
          {!isInTelegram && (
              ...
              <div className="bg-yellow-50 border border-yellow-200 rounded-xl px-4 py-4">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-yellow-800 mb-1">{t("emailNotVerifiedTitle")}</p>
                    <p className="text-xs text-yellow-700 leading-relaxed mb-3">
                      {t("emailNotVerifiedBody")}
                    </p>
                    {resent ? (
                      <p className="text-xs text-green-700 font-medium">{t("resent")}</p>
                    ) : (
                      <button
                        type="button"
