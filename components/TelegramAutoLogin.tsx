...
// Routes the user is allowed to land on directly, even from a fresh
// Mini App context where they're not yet authenticated. Anything else
// triggers the login dance below.
const PUBLIC_ROUTES = ["/", "/mentors", "/become-mentor"]

const isPublicRoute = (path: string) =>
  PUBLIC_ROUTES.includes(path) || path.startsWith("/mentors/")
...
export default function TelegramAutoLogin() {
    ...
    async function runLogin(role?: Role) {
      try {
        ...
        if (result.ok) {
          ...
          // First-time signup → onboarding; returning user → just stay
          // wherever they landed (don't yank them off /mentors/123).
          if (result.created) {
            router.push(me.role === "mentor" ? "/onboarding/mentor" : "/onboarding/student")
          } else if (!isPublicRoute(pathname)) {
            // Returning user opening a private route — let it render now
            // that we have a token. No navigation needed; React Suspense
            // / data fetches will pick up the new auth header.
          }
        }
      } catch (e: unknown) {
  ...
  }, [isInTelegram, initData, pathname, router])
